...
class isomod(object):
    ...
    def update(self):
        ...
        for i in range(self.nmod):
            ...
            if self.mtype[i] == 1:
                nlay = 1
                if self.thickness[i] > 5:
                    nlay = 20
                self.vs[:nlay, i]       = self.cvel[0, i]
                self.hArr[:nlay, i]     = self.thickness[i] / nlay
                self.nlay[i]            = nlay
            # B spline model
            elif self.mtype[i] == 2:
                self.isspl[i]               = False
    ...
    def update_depth(self):
        ...
        for i_mod in range(self.nmod):
            ...
            self.zArr[:self.nlay[i_mod],i_mod] = self.hArr[:self.nlay[i_mod],i_mod].cumsum()+self.hArr[:,:i_mod].flatten().sum()
        return True
    ...
    def get_vmodel(self):
        ...
        depth               = hArr.cumsum()
        vs, vp, rho = self.fixtopsed(vs, vp, rho)
        vs, vp = self.fixdeepmodel(hArr, vs, vp)
        rho[vp > 7.5]       = 3.35
        return hArr, vs, vp, rho, qs, qp, nlay

    @staticmethod
    def fixdeepmodel(hArr, vs, vp, mindep=10):
