...
ind = int(sys.argv[1])
NMS = 'N'
outname = "JdF_Profile_both/water_%s%g.mod"%(NMS,ind)
...
sed_c = 0.48
crust_thk = 7.
outdisp_ph = "JdF_Profile_both/%s%g_disp_ph.txt"%(NMS,ind)
outdisp_gr = "JdF_Profile_both/%s%g_disp_gr.txt"%(NMS,ind)
h5file = '/work3/wang/code_bkup/AgeJdF/Age_dset_everysec_both.h5'

if __name__ == "__main__":
    dset = Comp_Age_Tomo.CompSurfVel(h5file)
    pers = np.linspace(6.,30.,25)
    dispfile_ph = open(outdisp_ph,'w')
    dispfile_gr = open(outdisp_gr,'w')
    for period in pers:
        if dset['3_trajs']['%g_sec'%(period)]['vels_%s_ph'%(NMS)].value[ind-1] != 0.:
            dispfile_ph.write("{} {} {} \n".format(period, dset['3_trajs']['%g_sec'%(period)]['vels_%s_ph'%(NMS)].value[ind-1], 0.05))
        if dset['3_trajs']['%g_sec'%(period)]['vels_%s_gr'%(NMS)].value[ind-1] != 0.:
            dispfile_gr.write("{} {} {} \n".format(period, dset['3_trajs']['%g_sec'%(period)]['vels_%s_gr'%(NMS)].value[ind-1], 0.1))
    dispfile_ph.close()
    dispfile_gr.close()
    age = dset['3_trajs']['%g_sec'%(6.)]['ages_%s'%(NMS)].value[ind-1]
    water_dep = dset['3_trajs']['%g_sec'%(6.)]['deps_%s'%(NMS)].value[ind-1] / -1.e3
    sed_thk = dset['3_trajs']['%g_sec'%(6.)]['seds_%s'%(NMS)].value[ind-1] / 1.e3
    
    Ye_name = "%.1f_result_model.f2"%(Ye_arr[np.argmin(np.abs(Ye_arr - age))])
