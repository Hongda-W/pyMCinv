...
#            self.model.decompose()
    ...
    def get_period(self):
        ...
        if self.data.dispR.npper>0:
            ...
            self.TRpiso = np.array([])
        if self.data.dispR.ngper>0:
            self.TRgiso     = self.data.dispR.gper.copy()
        else:
            self.TRgiso = np.array([])
        if self.data.dispL.npper>0:
            self.TLpiso     = self.data.dispL.pper.copy()
        else:
            self.TLpiso = np.array([])
        if self.data.dispL.ngper>0:
            self.TLgiso     = self.data.dispL.gper.copy()
        else:
            self.TLgiso = np.array([])
        self.TRiso = np.unique(np.append(self.TRgiso,self.TRpiso))
        self.TLiso = np.unique(np.append(self.TLgiso,self.TLpiso))
        return
    ...
    def compute_fsurf(self, wtype='ray'):
        ...
        if wtype=='r' or wtype == 'rayleigh' or wtype=='ray':
            ...
            self.data.dispR.pvelp   = cr0[:nper][np.isin(self.TRiso,self.TRpiso)]
            self.data.dispR.gvelp   = ur0[:nper][np.isin(self.TRiso,self.TRgiso)]
        elif wtype=='l' or wtype == 'love':
            ilvry                   = 1
            nper                    = self.TLiso.size
            per                     = np.zeros(200, dtype=np.float64)
            per[:nper]              = self.TLiso[:]
            (ur0,ul0,cr0,cl0)       = fast_surf.fast_surf(self.model.nlay, ilvry, \
                                        self.model.vpv, self.model.vsv, self.model.rho, self.model.h, qsinv, per, nper)
            self.data.dispL.pvelp   = cl0[:nper][np.isin(self.TLiso,self.TLpiso)]
            self.data.dispL.gvelp   = ul0[:nper][np.isin(self.TLiso,self.TLgiso)]
        return
    ...
    def mc_joint_inv_iso(self, outdir='./workingdir', dispdtype='ph', wdisp=0.2, rffactor=40.,\
        ...
        ========================================================================================================
        """
        pathlib.Path(outdir).mkdir(parents=False, exist_ok=True)
        #-------------------------------
        # initializations
        #-------------------------------
        self.get_period()
        ...
        if init_run:
            ...
            if newmod.mtype[0] == 5: # water layer, added May 16th, 2018
                ...
                m1  += 1
            igood       = 0
            while ( not newmod.isgood(m0, m1, 1, 0, 3, 3)):
                igood   += igood + 1
                newmod  = copy.deepcopy(self.model.isomod)
                newmod.para.new_paraval(0)
                newmod.para2mod()
            ...
            if (wdisp >= 0. and wdisp <=1.):
                ...
                if monoc:
                    ...
                    if newmod.mtype[0] == 5: # water layer, added May 16th, 2018
                        ...
                        m1  += 1
                    itemp   = 0
                    while (not newmod.isgood(m0, m1, 1, 0, 3, 3)) and itemp < 100:
                        itemp       += 1
                        newmod      = copy.deepcopy(self.model.isomod)
                        newmod.para.new_paraval(1)
                        newmod.para2mod()
                        newmod.update()
                    if not newmod.isgood(m0, m1, 1, 0, 3, 3):
    ...
    def mc_joint_inv_iso_mp(self, outdir='./workingdir', dispdtype='ph', wdisp=0.2, rffactor=40., monoc=True, \
        ...
        if merge:
            ...
            for i in range(Nvpr):
                ...
                os.remove(invfname)
            outmodarr           = outmodarr.reshape(numbrun, int(outmodarr.size/numbrun))
            outdisparr_ph       = outdisparr_ph.reshape(numbrun, int(outdisparr_ph.size/numbrun))
            outdisparr_gr       = outdisparr_gr.reshape(numbrun, int(outdisparr_gr.size/numbrun))
            outrfarr            = outrfarr.reshape(numbrun, int(outrfarr.size/numbrun))
    ...
    def mc_joint_inv_iso_mp_new(self, outdir='./workingdir', dispdtype='ph', wdisp=0.2, rffactor=40., isconstrt=True, pfx='MC', \
