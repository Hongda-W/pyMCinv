...
class Field2d(object):
    ...
    def gradient(self, method='diff', edge_order=1, order=2):
        ...
        if method=='diff':
            ...
            self.grad   = np.gradient( self.Zarr, self.dlat_kmArr[0,0], self.dlon_kmArr[0,0], edge_order=edge_order)
            self.grad[0] = self.grad[0] * self.dlat_kmArr[0,0] / self.dlat_kmArr
            self.grad[1] = self.grad[1] * self.dlon_kmArr[0,0] / self.dlon_kmArr
    ...
    def check_curvature(self, workingdir, outpfx='', threshold=0.005):
        ...
        with open(tempGMT,'w') as f:
            ...
            if self.dlon == self.dlat:
                ...
                f.writelines('surface %s -T0.0 -G%s -I%g/%g %s \n' %( outfname, grdfile, self.dlon, self.dlat, REG ))
            ...
            if self.dlon == self.dlat:
                ...
                f.writelines('surface %s -T0.2 -G%s -I%g/%g %s \n' %( outfname, grdfile+'.T0.2', self.dlon, self.dlat, REG ))
