...
class model1d(object):
    ...
    def get_para_model(self, paraval, waterdepth=-1., vpwater=1.475, age=0.95, nmod=3, numbp=np.array([2, 4, 5]),\
                mtype = np.array([4, 2, 2]), vpvs = np.array([2., 1.75, 1.75]), maxdepth=50.):
        """
        ...
        ======================================================================================
        """
        self.isomod.init_arr(nmod=nmod, age=age)
    ...
    def get_grid_mod(self):
        ...
        for i in range(self.isomod.nmod-1):
            ind_dis = np.where(abs(self.zArr - depth_dis[i])<1e-4)[0]
            if ind_dis.size != 2:
                print(depth_dis)
                print(self.zArr)
        ...
        #
        self.grid_zArr        = self.zArr[indgrid_out]
        self.grid_VsvArr      = self.VsvArr[indgrid_out]
        return
